	...
	*)
	...
	if [[ $GEOIP == 'y' ]]; then
			...
			if [[ ! -f /usr/local/etc/GeoIP.conf ]]; then
				...
				wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/conf/GeoIP.conf
	...
	if [[ ! -e /etc/nginx/nginx.conf ]]; then
		mkdir -p /etc/nginx
		cd /etc/nginx || exit 1
		wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/conf/nginx.conf
	fi
	...
	if [[ $HTTP3 == 'y' ]]; then
		...
		wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/patches/nginx-http3-1.19.7.patch -O nginx-http3.patch
	...
	if [[ $HPACK == 'y' ]]; then
		if [[ $HTTP3 == 'n' ]]; then
			...
			wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/patches/nginx_hpack_push_with_http3.patch -O nginx_http2_hpack.patch
		fi
	...
	if [[ ! -e /lib/systemd/system/nginx.service ]]; then
		...
		wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/conf/nginx.service
	...
	if [[ ! -e /etc/logrotate.d/nginx ]]; then
		...
		wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/conf/nginx-logrotate -O nginx
	fi
...
2) # Uninstall Nginx
	...
	;;
3) # Update the script
	wget https://raw.githubusercontent.com/hwman/nginx-autoinstall/master/nginx-autoinstall.sh -O nginx-autoinstall.sh
